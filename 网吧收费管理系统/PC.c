 ...
 *********************************************************
//PC主菜单
void PCmenu(pList pcList, int page, int option) {
	pList p = getPCs()->next;
	if (10>=option)
	{
		page += option / 10;
	...
	printf("\n                     ");
	prOption("首  页", option == -5, 9);
	printf("   ");
	prOption("上一页", option == -4, 9);
	printf("   ");
	prOption("下一页", option == -3, 9);
	printf("   ");
	prOption("尾  页", option == -2, 9);
	printf("   ");
	int in = getch();
	key k = isKey(in);
	switch (k)
	...
	case tab:
		if (0<=option)
		{
			PCmenu(pcList, page, -5);
		}
		else
		{
			PCmenu(pcList, page, 0);
		}
		return;
	case left:
		if (option<0)
		{
			if (-4==option)
			{
				PCmenu(pcList, page, 9);
			}
			else {
				PCmenu(pcList, page, option - 5);
			}
		}
		else if (0!=page)
		{
			PCmenu(pcList, page - 1, option);
		}
		else
		{
			PCmenu(pcList, page, option);
	...
	case right:
		if (option<0)
		{
			PCmenu(pcList, page, option + 1);
		}
		else if (page > pcLength / 10 -1) {
			PCmenu(pcList, page, option);
		}
		else
		{
	...
	case enter:
		if (-1==option)
		{
			PCmenu(selectPC(0, getDefaultCriteriaPC(), pcList), 0, 0);
		}
		else if (-5==option)
		{
			PCmenu(pcList, 0, option);
		}
		else if (-4==option)
		{
			PCmenu(pcList, page - 1,option);
		...
		{
			PCmenu(pcList, pcLength / 10, option);
		}
		else
		{
			showPC(op, 1);
